...
export default function BillingHistory() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'paid':
        return 'text-green-500 bg-green-500/15 border border-green-500/30'
      case 'pending':
        return 'text-amber-400 bg-amber-400/15 border border-amber-400/30'
      case 'failed':
        return 'text-red-500 bg-red-500/15 border border-red-500/30'
      case 'refunded':
        return 'text-muted-foreground bg-muted border border-muted-foreground/20'
      default:
        return 'text-muted-foreground bg-muted border border-muted-foreground/20'
    }
  }
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-foreground mb-2">Billing History</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-card rounded-lg shadow p-6">
            <div className="flex items-center space-x-2 mb-2">
              ...
              <h3 className="font-semibold text-foreground">This Year</h3>
            </div>
            <p className="text-2xl font-bold text-foreground">
          ...
          <div className="bg-card rounded-lg shadow p-6">
            <div className="flex items-center space-x-2 mb-2">
              ...
              <h3 className="font-semibold text-foreground">Paid Invoices</h3>
            </div>
            <p className="text-2xl font-bold text-foreground">{summary.paid_invoices}</p>
          ...
          <div className="bg-card rounded-lg shadow p-6">
            ...
            <p className="text-2xl font-bold text-foreground">{summary.pending_invoices}</p>
            ...
            <div className="flex items-center space-x-4">
              <div className="relative">
                ...
                <input
                  ...
                  className="pl-10 pr-4 py-2 border border-input bg-background text-foreground placeholder:text-muted-foreground rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                />
              </div>
              
              <button
                onClick={() => setShowFilters(!showFilters)}
                className="flex items-center space-x-2 px-4 py-2 text-foreground bg-muted rounded-lg hover-surface"
              >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 border border-input bg-background text-foreground rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  >
                ...
                <div className="flex items-end">
                  <button
                    onClick={() => setFilters({})}
                    className="px-4 py-2 text-foreground bg-muted rounded-lg hover-surface"
                  >
                    Clear Filters
                  </button>
                </div>
      ...
      <div className="bg-card rounded-lg shadow overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-8 text-center text-muted-foreground">
            <CreditCard className="w-12 h-12 mx-auto mb-4 text-muted-foreground/60" />
            ...
            <table className="min-w-full divide-y divide-border">
              ...
              <tbody className="bg-card divide-y divide-border">
                {invoices.map((invoice: Invoice) => (
                  <tr key={invoice.id} className="hover-surface">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-foreground">{invoice.plan_name}</div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-foreground">{formatDate(invoice.invoice_date)}</div>
                      {invoice.paid_date && (
                        <div className="text-sm text-muted-foreground">
