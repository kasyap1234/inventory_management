...
func (r *userRoleRepo) Create(ctx context.Context, tenantID uuid.UUID, userRole *models.UserRole) error {
	query := `
		INSERT INTO user_roles (user_id, role_id, tenant_id, is_active, assigned_at, created_at)
		...
		ON CONFLICT (user_id, tenant_id, role_id) DO UPDATE SET is_active = true, assigned_at = NOW()
	`
