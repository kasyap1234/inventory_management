...
func (h *OrderHandlers) validateUUID(idStr string) (uuid.UUID, error) {
	...
	if err != nil {
		httpErr := echo.NewHTTPError(http.StatusBadRequest, "Invalid UUID format")
...
func (h *OrderHandlers) GetOrderAnalytics(c echo.Context) error {
	...
	if startDateStr == "" {
		...
		if err != nil {
			return common.SendValidationError(c, "start_date", "Invalid date value: "+err.Error())
		}
	}
	...
	if endDateStr == "" {
		...
		if err != nil {
			return common.SendValidationError(c, "end_date", "Invalid date value: "+err.Error())
		}
	}
