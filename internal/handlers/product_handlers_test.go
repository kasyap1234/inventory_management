	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectError {
				...
				// The error message is now a structured map
				...
				// Check that details exist and contain the validation error
				details, ok := errorObj["details"].(map[string]string)
				require.True(t, ok, "Details should exist")
				_, hasIDField := details["id"]
				assert.True(t, hasIDField, "Details should have 'id' field")
